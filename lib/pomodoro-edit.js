  ...
  cm.showHint({
    hint: () => {
      ...
      CodeMirror.on(result, 'pick', item => {
        const cs = [`${bullet} [ ] `, `${bullet} [] `];

        const text = cm.doc.getLine(pos.line);
			  const found = text.match(/^( *)/);
			  const startCh = found ? found[1].length : 0;

        const trimmedText = text.substring(startCh, pos.ch);
        let deleteCnt = 0;
			  if (cs.some(c => c.indexOf(trimmedText) === 0)) {
          cm.doc.replaceRange('', {line: pos.line, ch: startCh}, {line: pos.line, ch: pos.ch});
          deleteCnt = pos.ch - startCh;
        }

        switch (item.text) {
          case candidates[0].text:
            cm.setCursor(CodeMirror.Pos(pos.line, pos.ch + bullet.length + 14 - deleteCnt));
            _showLoopHint(cm);
            break;
          case candidates[1].text:
            cm.setCursor(CodeMirror.Pos(pos.line, pos.ch + bullet.length + 15 - deleteCnt));
            _showLoopHint(cm);
            break;
          ...
          case candidates[3].text:
            cm.setCursor(CodeMirror.Pos(pos.line, pos.ch + bullet.length + 8 - deleteCnt));
            _showTimeHint(cm);
            break;
        }
      });
