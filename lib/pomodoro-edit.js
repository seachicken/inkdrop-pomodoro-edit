  ...
  _disposables.add(inkdrop.commands.add(document.body, "core:save-note", () => {
    ...
    _core.findAndStartTimer(editingNote.body, editingNote._id, {
      start: ptext => {
        ...
        _updateTrayOnInterval(remaining, ptext);
      },
	    finish: ptext => {
        const notification = new Notification('🍅 Finished!', {
          body: ptext.content
        });
        notification.onclick = () => _goToLineToExtraTime(ptext);
      },
      cancel: () => {
        _updateTrayOnCancel();
      }
...
function _updateTrayOnStart(ptext) {
  ...
  const time = _getDisplayTime(ptext.time + ptext.extraTime);
...
function _updateTrayOnInterval(remaining, ptext) {
  _configureMenu(ptext);
...
function _goToLineToCheckbox(ptext) {
  _goToLine(ptext, ptext.checkboxCh);
}

function _goToLineToExtraTime(ptext) {
  _goToLine(ptext, ptext.extraTimeCh);
}
