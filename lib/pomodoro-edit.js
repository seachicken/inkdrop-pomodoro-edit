  ...
  _disposables.add(inkdrop.commands.add(document.body, "core:save-note", () => {
    ...
    _core.findAndCountPomodoroText(editingNote.body, editingNote._id, {
      start: ptext => {
        ...
        notification.onclick = () => _goToLine(ptext);
      },
      cancel: () => {
        _updateTrayOnCancel();
      }
...
function _configureMenu(ptext) {
  _tray.setContextMenu(remote.Menu.buildFromTemplate([
      { label: ptext.content,
        submenu: [
          { label: 'Go to Line', click: () => _goToLine(ptext) },
...
function _goToLine(ptext) {
  ...
  const interval = setInterval(() => {
    ...
    if (inkdrop.activeEditor.props.noteId === ptext.id) {
      ...
      inkdrop.activeEditor.cm.setCursor(ptext.line, ptext.ch)
